  ...
  render() {
      ...
      <ol className="fav-list">
        {this.props.profiles.map((profile) => {
          ...
          return <li key={profile.id} className="fav-list-item">{`${userName}'s`} favorite movie is {favMovie}</li>
  		})}
      </ol>
    );
  }
...
/*
//draft
riskNamesArr =  ["Time", "Riskchanged", "Scope", "Risk4", "Test", "Test(2)"];

riskWorkflowStateArr =  ["In Bearbeitung", "Abgeschlossen", "In Bearbeitung", "Abgeschlossen", "Geplant", "In Bearbeitung"];

this.testArr = this.riskNamesArr.map( (x, i) => {
    console.log(("name: " + x +"state: " + this.riskWorkflowStateArr[i])
